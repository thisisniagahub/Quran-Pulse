import type { ChatMessage, StudyPlan, TajweedSession, JawiConversion, IqraPracticeSession, Surah } from '../types';

const DB_NAME = 'quranPulseDB';
const DB_VERSION = 4; // Version incremented for schema changes
...
const openDB = (): Promise<IDBDatabase> => {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            // New store for caching surah data
            if (!db.objectStoreNames.contains('surahCache')) {
                db.createObjectStore('surahCache', { keyPath: 'number' });
            }
        };
    });
};
...
const get = <T>(storeName: string, key: any): Promise<T> => performDBAction<T>(storeName, 'readonly', store => store.get(key));
...
// --- Surah Cache ---
export const addSurahToCache = (surah: Surah) => performDBAction('surahCache', 'readwrite', store => store.put(surah));
export const getSurahFromCache = (surahNumber: number): Promise<Surah | null> => get<Surah | null>('surahCache', surahNumber);