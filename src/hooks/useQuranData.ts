...
import { addSurahToCache, getSurahFromCache } from '../services/dbService';
...
export const useQuranData = (surahNumber: number) => {
    ...
    const fetchSurah = useCallback(async (number: number) => {
        ...
        try {
            // 1. Check cache first
            const cachedSurah = await getSurahFromCache(number);
            if (cachedSurah) {
            ...
            // 3. Set state and add to cache
            setSurah(combinedSurah);
            await addSurahToCache(combinedSurah); // Cache the new data
