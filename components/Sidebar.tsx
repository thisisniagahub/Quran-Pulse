...
import {
  ...
  CalendarDaysIcon, WrenchScrewdriverIcon, XIcon, LightbulbIcon, ImageIcon
} from './icons/Icons';
...
export const sidebarSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { view: ActiveView.IMAGE_EDITOR, label: 'Editor Imej', icon: <ImageIcon className="w-5 h-5" /> },
    ],
  },
  {
...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen, onClose }) => {
  // FIX: Replaced the extremely long base64 string with a placeholder SVG path
  // and added a comment about proper asset handling. The previous string likely
  // caused parsing issues.
  const logoSrc = "/quranpulse-logo.svg"; // Assuming you have an SVG logo at this path or similar

  const sidebarContent = (
    <>
        <div className="flex items-center justify-center h-16 px-4 border-b border-border flex-shrink-0">
            <img src={logoSrc} alt="QuranPulse Logo" className="h-10 object-contain" />
